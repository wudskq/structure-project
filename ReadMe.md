...
- 核心思想: 遇到数据直接输出,遇到运算符,如何操作符栈为空或者栈顶为左括号,则当前操作符直接push进入操作符栈中

  如果当前操作符为右括号,直接pop弹出操作符栈中的所有操作符,直到遇到左括号为止,左括号只弹出不输出

  如果当前操纵符为其他操作符,比较当前操作符与栈顶操作符优先级,如果当前操作符比栈顶操作符优先级高,则一直pop弹出操作符栈中的操作符,直到当前操作符优先级小于栈顶运算符优先级

  直到pop弹出到操作符栈的末尾,则将栈中的数据依次弹出
